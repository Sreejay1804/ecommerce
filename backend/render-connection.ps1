...
function Prepare-RenderDeployment {
    ...
    # Check Maven wrapper
    if (Test-Path "mvnw") {
        Write-Success "[SUCCESS] Maven wrapper found"
    } else {
        Write-Warning "Maven wrapper (mvnw) not found. You may need to run: mvn wrapper:wrapper"
    }
...
function Test-JavaApplication {
    ...
    if (Test-Path "mvnw") {
        ...
        Write-Warning "Maven wrapper not found. Run: mvn test"
    }
}
